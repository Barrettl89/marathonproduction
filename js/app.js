  ...
  function renderModelTable() {
    ...
    // Each group: {label, color, arr, typeLabel (shown as pill)}
    var groups = [
      {label:'Residential — Single Section', color:'#4ade80', typeLabel:'Single', arr: (function(){ var a=[]; for(var k=0;k<modelData.length;k++){if(modelData[k].type==='Single-Section')a.push(k);} return a; })()},
      {label:'Residential — Double Section', color:'#60A5FA', typeLabel:'Double', arr: (function(){ var a=[]; for(var k=0;k<modelData.length;k++){if(modelData[k].type==='Double-Section')a.push(k);} return a; })()},
      {label:'Workforce',  color:'#f97316', typeLabel:'Workforce',  arr: (function(){ var a=[]; for(var k=0;k<workforceData.length;k++)a.push(k); return a; })()},
      {label:'Park Model', color:'#a78bfa', typeLabel:'Park Model', arr: (function(){ var a=[]; for(var k=0;k<parkModelData.length;k++)a.push(k); return a; })()},
    ];

    // Section filter maps to which groups to show
    var showGroup = function(label) {
      if (!modelSectionFilter) return true;
      if (modelSectionFilter === 'Residential') return label.indexOf('Residential') === 0;
      return label === modelSectionFilter || label.indexOf(modelSectionFilter) === 0;
    };

    groups.forEach(function(g) {
      if (!showGroup(g.label)) return;

      var src = (g.label.indexOf('Workforce') >= 0) ? workforceData
               : (g.label.indexOf('Park Model') >= 0) ? parkModelData
               : modelData;

      // Sort indices by sf
      var idxs = g.arr.slice();
      idxs.sort(function(a,b){ return (src[a].sf||0)-(src[b].sf||0); });
      ...
      idxs.forEach(function(j) {
        ...
        if (!sectionVisible) {
          ...
          tbody.insertAdjacentHTML('beforeend',
            ...
            '<td colspan="9" style="padding:10px 16px 6px;font-size:10px;font-weight:800;' +
            'letter-spacing:0.14em;text-transform:uppercase;color:' + g.color + ';' +
            'border-bottom:1px solid #2a2a3e;">' + g.label + '</td></tr>');
        }

        visible++;
        var bedNum = row.beds || 0;
        var bedColor = bedNum <= 2 ? 'pill-gray' : bedNum === 3 ? 'pill-blue' : 'pill-green';
        var pct = Math.min(100, Math.round(((row.sf||0)/maxSqft)*100));
        var pillColor = g.label.indexOf('Single') >= 0 ? 'pill-green'
                      : g.label.indexOf('Double') >= 0 ? 'pill-blue'
                      : g.label.indexOf('Workforce') >= 0 ? 'pill-orange'
                      : 'pill-purple';
        var notesText = row.notes || '&mdash;';
        var notesStyle = row.notes ? 'normal' : 'italic';
        var rowLen = row.length || '';

        // Only modelData rows are editable (workforceData/parkModelData are read-only for now)
        var isModelData = (src === modelData);
        var editIdx = isModelData ? j : -1;
        var isEditing = (isModelData && modelEditingIndex === j);

        if (isEditing) {
          tbody.insertAdjacentHTML('beforeend',
            '<tr id="ml-row-' + j + '" class="sn-editing">' +
          ...
          tbody.insertAdjacentHTML('beforeend',
            ...
            '<td class="fc-action-cell" style="white-space:nowrap;">' +
            (isModelData ? '<button class="btn-edit" onclick="editModelRow(' + j + ')">&#9998; Edit</button>' : '<span style="color:var(--subtext);font-size:10px;">—</span>') +
            '</td></tr>');
        }
      });
    });

    var total = modelData.length + workforceData.length + parkModelData.length;
    document.getElementById('model-count').textContent = visible + ' of ' + total + ' models';
  }
